...
export default function AdminVillages() {
  ...
  const filteredVillages = villages.filter(village => {
    const matchesSearch = village.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         village.location.toLowerCase().includes(searchTerm.toLowerCase());
    const matchesType = filterType === 'all' || village.type === filterType;
  ...
  const handleDeleteVillage = async (villageId) => {
    ...
    try {
      ...
      if (data.success) {
        setVillages(villages.filter(vill => vill.id !== villageId));
                          ...
                          <div className="flex items-center">
                            <div className="flex-shrink-0 h-10 w-10">
                              <img 
                                ...
                                src={village.img_sm || '/placeholder.jpg'} 
                                alt={village.title}
                              />
                            </div>
                            <div className="ml-4">
                              <div className="text-sm font-medium text-gray-900">{village.title}</div>
                              <div className="text-sm text-gray-500">{village.short_description?.substring(0, 50)}...</div>
                            </div>
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-indigo-100 text-indigo-800">
                            {village.type === 'desa-wisata' ? 'Desa Wisata' : village.type}
                          </span>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{village.location}</td>
                ...
                <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                  <div>
                    <p className="text-sm text-gray-700">
                      Showing <span className="font-medium">1</span> to <span className="font-medium">{filteredVillages.length}</span> of{' '}
                      <span className="font-medium">{filteredVillages.length}</span> results
                    </p>
                  </div>
                  <div>
