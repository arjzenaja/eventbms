...
export default function AdminCulinary() {
  ...
  useEffect(() => {
    const fetchCulinaryItems = async () => {
      try {
        ...
        if (data.success) {
          setCulinaryItems(data.culinaryItems);
        } else {
  ...
  const handleDeleteCulinaryItem = async (itemId) => {
    ...
    try {
      const response = await fetch(`/api/culinary/${itemId}`, {
        method: 'DELETE',
      });
                      ...
                      <tr key={item.id}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                            {item.type === 'kuliner' ? 'Kuliner' : item.type}
                          </span>
                        </td>
                ...
                <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                  <div>
                    <p className="text-sm text-gray-700">
                      Showing <span className="font-medium">1</span> to <span className="font-medium">{filteredCulinaryItems.length}</span> of{' '}
                      <span className="font-medium">{filteredCulinaryItems.length}</span> results
                    </p>
                  </div>
                  <div>
