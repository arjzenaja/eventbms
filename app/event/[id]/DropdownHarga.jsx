    ...
    <div className="flex flex-col gap-6">
      <div className="bg-[#23262e] rounded-xl p-6 shadow border border-[#2a2d36]">
        <label className="font-bold text-xl text-white flex items-center gap-2 mb-4">
          <FaTags className="text-blue-600 text-2xl" /> Pilih Paket
        </label>
        <select
                      className="bg-[#181a20] text-white rounded px-4 py-3 pr-12 border border-[#444] focus:outline-none focus:ring-2 focus:ring-blue-600 w-full text-lg mb-2 appearance-none relative"
        ...
        {selectedPkg.prices && (
          <select
            className="bg-[#181a20] text-white rounded px-4 py-2 border border-[#444] focus:outline-none focus:ring-2 focus:ring-blue-600 w-full text-base mt-2 appearance-none relative"
            value={selectedType}
      ...
      <div className="bg-[#23262e] rounded-xl p-6 shadow border border-[#2a2d36] flex flex-col md:flex-row items-center justify-between gap-6">
        <div className="flex items-center gap-6 flex-1">
          ...
          <div>
            ...
            {selectedPkg.type && (
              <div className="text-xs text-blue-600 font-semibold">Tipe: {selectedPkg.type}</div>
            )}
          </div>
        </div>
        <div className="flex flex-col items-end gap-2 min-w-[180px]">
          <span className="text-blue-300 font-bold text-2xl">Rp {price?.toLocaleString("id-ID")}</span>
          <button className="mt-3 bg-blue-600 hover:bg-blue-700 text-white font-bold px-6 py-2 rounded-lg transition-all duration-200 shadow hover:scale-105">Beli Sekarang</button>
        </div>
      </div>
    </div>
  );
