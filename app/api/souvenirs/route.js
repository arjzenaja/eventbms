...
export async function GET() {
  try {
    ...
    // Get oleh_oleh data from master data section
    const olehOlehItems = dbData.oleh_oleh || [];
    ...
    return NextResponse.json(
      {
        success: false,
        message: 'Terjadi kesalahan saat mengambil data oleh-oleh'
      },
      { status: 500 }
    );
  }
...
export async function POST(request) {
  try {
    ...
    const type = formData.get('type') || 'makanan';
    const price_range = formData.get('price_range') || '25.000 - 100.000';
    const category = formData.get('category') || 'Makanan';
    const contact = formData.get('contact') || '';
    const address = formData.get('address') || location;
    const features = formData.get('features') || ['Produk Lokal', 'Kualitas Terjamin'];
    ...
    if (img_sm && img_sm instanceof File) {
      ...
      const img_sm_filename = `souvenir_sm_${Date.now()}${img_sm_ext}`;
    ...
    if (img_lg && img_lg instanceof File) {
      ...
      const img_lg_filename = `souvenir_lg_${Date.now()}${img_lg_ext}`;
    ...
    // Generate new ID
    const validIds = (dbData.oleh_oleh || [])
      .map(item => {
    ...
    // Create new oleh_oleh item
    const newOlehOlehItem = {
      id: newId,
      img_sm: img_sm_path,
      img_lg: img_lg_path,
      title: title,
    ...
    // Add to oleh_oleh array
    dbData.oleh_oleh.push(newOlehOlehItem);
    ...
    return NextResponse.json({
      success: true,
      message: 'Item oleh-oleh berhasil ditambahkan',
      olehOlehItem: newOlehOlehItem
    });
  } catch (error) {
    console.error('Error creating oleh_oleh item:', error);
    return NextResponse.json(
      {
        success: false,
        message: 'Terjadi kesalahan saat membuat item oleh-oleh: ' + error.message
      },
      { status: 500 }
    );
  }
